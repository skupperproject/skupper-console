...
class Adapter {
    ...
    this.data.services.forEach(
      (service: {
        ...
        if (service.requests_handled) {
          service.requests_handled.forEach((request) => {
            for (const server in request.by_server) {
              ...
              this.addTarget(service.targets as any, service.address, request.site_id);
            }
          });
          ...
          if (service.connections_egress.length > 0) {
            service.connections_egress.forEach((egress) => {
              for (const connection_id in egress.connections) {
                ...
                this.addTarget(service.targets as any, connection.server, egress.site_id);
              }
        ...
        if (service.targets[0]) {
          this.addTarget(service.targets as any, service.address, service.targets[0].site_id);
        }
      },
    );
  };
  ...
  newService = ({ address, protocol = 'http', client, site_id }: any) => {
    const service: any = {
      derived: !this.data.emptyHttpServices.some((s: { address: any }) => s.address === address),
    ...
    this.data.services.forEach(
      (service: {
        ...
        if (service.requests_received) {
          service.requests_received.forEach((request: { by_client: any; site_id: any }) => {
            for (const clientKey in request.by_client) {
              ...
              if (!found) {
                ...
                this.addTargetToService(newService, utils.shortName(clientName), request.site_id);
              } else if (found.derived) {
                this.addTargetToService(found, utils.shortName(clientKey), request.site_id);
              }
            }
          });
        } else if (service.connections_egress) {
          service.connections_egress.forEach((connection_egress: { connections: any }) => {
            ...
            for (const egress_connection_id in connections) {
              service.connections_ingress.forEach(
                (connection_ingress: { site_id: any; connections: any }) => {
                  ...
                  if (Object.keys(ingress_connections).includes(egress_connection_id)) {
                    const client = ingress_connections[egress_connection_id].client;
                    const targetInfo = this.findClientInTargets(client, ingress_site_id);
          ...
          service.connections_ingress.forEach(
            (ingress: { connections: { [x: string]: { client: any } }; site_id: any }) => {
              for (const connectionID in ingress.connections) {
                ...
                const targetInfo = this.findClientInTargets(client, ingress.site_id);
                if (!targetInfo.target) {
                  const newService = this.newService({
                    address: client,
                    protocol: 'tcp',
          ...
          if (request.site_id === fromSite) {
            for (const client in request.by_client) {
              ...
              if (from === clientService) {
                utils.aggregateAttributes(request.by_client[client].by_handling_site[toSite], req);
              }
            }
          }
        }
  ...
  addSourcesTargets = () => {
    ...
    this.data.services.forEach((service: { sourceServices: any[]; targetServices: never[] }) => {
                    ...
                    if (sourceService) {
                      if (sourceService === sourceAddress) {
                        ...
                        if (req.start_time) {
                          ...
                          req = JSON.parse(JSON.stringify(egress.connections[connectionID]));
                        }
                      }
                    }
                  }
  ...
  findSite = (site_id: any) => this.data.sites.find((s: { site_id: any }) => s.site_id === site_id);
      ...
      (link: {
        ...
        if (
          ...
          const found = matrix.find(
            (r) =>
              r.ingress === link.source.service.address && r.egress === link.target.service.address,
          );
          if (found) {
  ...
  allServiceMatrix = (stat: string) => {
    const matrix: {
      ...
      address: any;
      messages: any;
      info: { source: { site_name: any; site_id: any }; target: { site_name: any; site_id: any } };
    }[] = [];
    if (!stat) {
      stat = 'bytes_out';
    }
              ...
              for (let i = 0; i < toService.connections_ingress.length; i++) {
                ...
                if (ingress.site_id === from_site_id) {
                  if (Object.keys(ingress.connections).includes(connectionID)) {
                    ...
                    const fromService = this.serviceFromServer(clientID, from_site_id);
  ...
  requestSums = (service: { [x: string]: any[] }, key: string | number) => {
    ...
    if (service[key]) {
      service[key].forEach((request: { site_id: any }) => {
        sums.push({
          site_name: this.siteNameFromId(request.site_id) || `${request.site_id} doesn't exist`,
