...
const ProcessPairs = function () {
    ...
    const [flowPairsQueryParamsPaginated, setFlowParisQueryParamsPaginated] =
        useState<RequestOptions>(initAllFlowParisQueryParamsPaginated);
    ...
    const { data: flowPairsPairsData, isLoading: isLoadingFlowPairsPairsData } = useQuery(
        [
            QueriesProcesses.GetFlowPairs,
            { ...initAllFlowParisQueryParamsPaginated, ...flowPairsQueryParamsPaginated },
        ],
        () =>
    ...
    const TcpFlowPairs = flowPairsPairsData.results.filter(({ protocol }) =>
        isTcp(protocol as AvailableProtocols),
    );

    const HttpFlowPairs = flowPairsPairsData.results.filter(
        ({ protocol }) => !isTcp(protocol as AvailableProtocols),
    );
            ...
            <Grid hasGutter>
                <GridItem>
                    <Breadcrumb>
                        ...
                        <BreadcrumbItem>
                            <Link
                                to={`${ProcessesRoutesPaths.Processes}/${processId}`}
                            >{`${processName}`}</Link>
                        </BreadcrumbItem>
